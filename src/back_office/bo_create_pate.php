...
if (isset($_POST) && !empty($_POST['name']) && !empty($_POST['description']) && isset($_POST['extra_price'])) {
    ...
    $sql = "INSERT INTO pizzas_pates (name, description, extra_price)
    ...
    if ($query->execute()) {
        $_SESSION['info_message'] = "La pâte a bien été ajoutée.";
    } else {
        $_SESSION['info_message'] = "Erreur lors de l'ajout de la pâte.";
    }
...
header('Location: test_create_pate.php'); // Redirection vers la page du formulaire
exit();
?>