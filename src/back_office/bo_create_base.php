...
if (isset($_POST) && !empty($_POST['name']) && !empty($_POST['description']) && isset($_POST['extra_price'])) {
    ...
    $sql = "INSERT INTO pizzas_bases (name, description, extra_price)
    ...
    if ($query->execute()) {
        $_SESSION['info_message'] = "La base a bien été ajoutée.";
    } else {
        $_SESSION['info_message'] = "Erreur lors de l'ajout de la base.";
    }
...
header('Location: test_create_base.php'); // Redirige vers la page du formulaire (ou une autre page)
