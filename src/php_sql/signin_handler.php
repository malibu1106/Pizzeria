...
if (isset($_POST) && !empty($_POST['first_name']) && !empty($_POST['last_name']) 
    && !empty($_POST['si_email']) && !empty($_POST['si_password']) && !empty($_POST['si_retyped_password'])) {

    try {
        // Assainir et hacher les données de formulaire
        $first_name = strip_tags($_POST['first_name']);
        $last_name = strip_tags($_POST['last_name']);
        $email = strip_tags($_POST['si_email']);
        $password = strip_tags($_POST['si_password']);
        $retyped_password = strip_tags($_POST['si_retyped_password']);

        // Vérifier que les deux mots de passe correspondent
        if ($password !== $retyped_password) {
            $_SESSION['info_message'] = "Les mots de passe ne correspondent pas.";
            header('Location: ../connexion.php'); // Redirection vers la page d'inscription
            exit();
        }
