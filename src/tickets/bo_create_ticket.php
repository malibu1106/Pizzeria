...
if (isset($_POST) && !empty($_POST['object']) && !empty($_POST['message'])) {
    ...
    // Vérifier si un fichier est uploadé et s'il y a une image à traiter
    if (!empty($_FILES['attachment']['name'])) {
        $image_url = strip_tags($_FILES['attachment']['name']);
        
        // GESTION DE L'IMAGE
        $uploadDir = '../tickets/attachments/'; // DOSSIER OU L'ON STOCKERA LES PIECES JOINTES
        $imageFileType = strtolower(pathinfo($image_url, PATHINFO_EXTENSION)); // EXTENSION DU FICHIER UPLOADE
        ...
        if (in_array($imageFileType, $allowedTypes)) { // SI L'EXTENSION DU FICHIER UPLOADE EST AUTORISEE
            ...
            header('Location: test_create_ticket.php');
            exit(); // Pour arrêter l'exécution en cas d'erreur
        }
    }
